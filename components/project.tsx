        ...
        <SlideIn direction={reverse ? "left" : "right"}>
            <div className={`project relative flex flex-wrap items-center gap-y-8 ${reverse ? "flex-row-reverse" : ""}`}>
                <div className="relative w-full lg:w-1/2 z-30">
                    ...
                    <Link href={`/projects/${slug}`} className={`block group relative top-[-50px] lg:top-0 w-full lg:w-[120%] rounded-xl overflow-hidden lg:mt-10 border-hemerald cursor-pointer transition-all ${reverse ? " border-r-2 border-l-2 lg:border-l-0 lg:translate-x-[-16%]" : "border-l-2 border-r-2 lg:border-r-0"}`}>
